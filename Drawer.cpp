...
void Drawer::Runner1() {
    ...
    while (gui.IsRunning()) {
        ...
        if (it != config.actual_adress.end()) {
            // CRITICAL: Reset trackers at the very start of processing
            ...
            if (hitStatus) {
                ...
                // If miss, get physical frame from page table (may trigger RAM eviction)
                unsigned long long p_frame = pages.getPhysicalAddress(vpn, offset, is_write, cache, x);
                // Insert into TLB (may trigger TLB eviction)
                cache.insert_with_LRU_algo(vpn, p_frame, true);
            }

            // Capture the index AFTER all potential movements/evictions
...
void Drawer::SPRINTER1() {
    ...
    while (gui.IsRunning()) {
        ...
        gui.Render(cache, pages, hitStatus, config, activeIndex, "Optimal Replacement");
    }
    cache.perfomance_metrics(config.latency, pages.ram_read_count, pages.ram_write_count, pages.dirty_evictions);
}
